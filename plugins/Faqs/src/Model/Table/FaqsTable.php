...
class FaqsTable extends AppTable
{
    ...
    public function initialize(array $config): void
    {
		...
		$this->addBehavior('MultiTranslatable', ['fields' => ['title', 'excerpt', 'text', 'string_1', 'seotitle', 'seodescription', 'seokeywords', 'modified', '_status']]);
		$this->addBehavior('SefUrl');
        $this->addBehavior('Sortable');

        $this->hasOne('Preview', ['className' => 'Images.Images'])
            ->setForeignKey('ref')
            ->setConditions(['Preview.model' => 'Faqs', 'Preview.scope' => 'preview']);
    ...
    {
        $key = $options['key'];
        return $query->where(['Faqs.title LIKE' => "%" . trim($key) . "%"]);
    }
}
